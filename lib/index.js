...
function calcurateTextPositionIntoRectangle({
  ctx,
  text,
  style,
  rect
  ...
  for (let fontSize = style.fontSize; fontSize > 0; fontSize -= 1) {
    ...
    while (words.length > 0) {
      let i;
      let slicedText;
      let textSize; // Remove words until the rest fit into the width.
    ...
    throw new Error('Failed a process to calculate text positions to draw texts into a rectangle. Change a smaller font size or shorten texts in the options and retry.');
  }
}
/**
...
async function generateOpenGraphImage(options) {
  ...
  const {
    ...
    style: _objectSpread({}, options.style.title),
    rect: {
  ...
  ctx.font = getFontStyle(_objectSpread({}, options.style.author, {
...
function timeout() {
  ...
  const handle = async (timeoutMs = 10000) => new Promise((_, reject) => {
    timeoutId = setTimeout(() => {
      reject(new Error('Occurred timeout error in the process to generate an image. Change a smaller font size or shorten texts in the options and retry.'));
    }, timeoutMs);
  });
  /**
   * Cancel timeout.
...
module.exports = async options => {
  const t = timeout();
  return Promise.race([generateOpenGraphImage(options), t.handle(options.timeout)]).then(output => {
    t.cancel();
    return output;
  }).catch(error => {
    t.cancel();
